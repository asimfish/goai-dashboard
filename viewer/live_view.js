...
function frameCamera(){scene.updateMatrixWorld(true);const box=new THREE.Box3();if(dynamic.size){for(const n of dynamic.values())box.expandByPoint(n.getWorldPosition(new THREE.Vector3()));}else box.setFromObject(root);const sphere=box.getBoundingSphere(new THREE.Sphere()),d=Math.max(1.5,sphere.radius/Math.sin(THREE.MathUtils.degToRad(camera.fov/2)));controls.target.copy(sphere.center);camera.position.copy(sphere.center).add(new THREE.Vector3(.7,-.85,.6).multiplyScalar(d));camera.near=Math.max(.01,d/1000);camera.far=d*10;camera.updateProjectionMatrix();controls.update();}
...
async function staticScene(id){
 document.body.classList.add('live');const name=id+'_scene',meta=await jsonFile('./'+name+'.geometry.json');await geometry('./'+name+'.glb',meta,meta.geometry_sha256);frameCamera();status.mode='static';$('mode').textContent='独立工作流场景 · 静态查看';$('scope').textContent='此场景暂未配置任务轨迹。';$('identity').textContent=meta.scene_id+' / '+meta.layout_version;loaded=true;
}
async function live(){
 document.body.classList.add('live');if(!/^wss?:\/\//.test(wsUrl))throw Error('WebSocket 地址无效');const file=params.get('geometry')||'./layout_v4_textured_draco.glb',meta=await jsonFile(file.replace(/\.glb$/,'.geometry.json'));
...
function animate(now){requestAnimationFrame(animate);const delta=Math.max(0,(now-lastNow)/1000);lastNow=now;
 ...
 controls.update();renderer.render(scene,camera);raf++;status.rendered_frames++;
...
const sceneId=params.get('scene');(wsUrl?live():sceneId?( /^wf0[1-5]$/.test(sceneId)?staticScene(sceneId):Promise.reject(Error('未知场景'))):replay()).catch(fail);requestAnimationFrame(animate);
